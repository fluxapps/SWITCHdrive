...
class ilDynamicLanguageOC {
	...
	/**
	 * @param ilDynamicLanguageInterfaceOD $parent_object
	 * @param int                          $mode
	 *
	 * @return mixed
	 */
	public static function getInstance(ilDynamicLanguageInterfaceOD $parent_object, $mode = self::MODE_PROD) {
	 ...
	 * @param int                          $mode
	 */
	protected function __construct(ilDynamicLanguageInterfaceOD $parent_object, $mode = self::MODE_PROD) {
		global $tpl;
		/**
		 * @var tpl ilTemplate
		 */
		...
		if ($this->mode == self::MODE_DEV AND $_GET['cmdMode'] != 'asynch') {
			...
			foreach (self::$missing as $key) {
				foreach ($this->languages as $lng) {
					...
					$echo .= "<a href='#' id='{$lng}_{$key}' data-type='text' data-pk='{$lng}/{$key}' data-url='{$url}' data-value='{$existing}'>{$lng}_{$key}</a><br>";
				}
			}
			$echo .= "<br>Used:<br>";
...
/**
 * Interface ilDynamicLanguageInterfaceOC
 */
interface ilDynamicLanguageInterfaceOC {
